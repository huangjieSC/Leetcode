...
然后循环运算二维数组中的每个点的最低血量，例如，对于dungeon[i][j]点来说，通过dungeon[i][j]本身需要的血量是-dungeon[i][j]，通过dungeon[i][j]点之后到达终点所需的最小血量为Math.min(dungeon[i + 1][j], dungeon[i][j + 1])，对两者求和，同样的，如果这求和值小于1那么应该以1记，完整递推公式为dungeon[i][j] = Math.max(1, -dungeon[i][j] + Math.min(dungeon[i + 1][j], dungeon[i][j + 1]))。
最后返回dungeon[0][0]的值即可。
        ...
        for (int i = rows - 2; i >= 0; i--){
            dungeon[i][cols-1] = Math.max(1, -(dungeon[i][cols-1]) + dungeon[i+1][cols-1]);
        }
