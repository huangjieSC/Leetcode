...
因为只需要一遍扫描，算法的时间复杂度是O(n)，空间复杂度是栈的空间，最坏情况是都是左括号，所以是O(n)。
    ...
    public static int longestValidParentheses(String s) {
        ...
        for (int i=0; i<s.length();i++){
            if (s.charAt(i)=='('){
                stack.push(i); // pay attention, we are pushing the index of left parenthesis into the stack
            }else{
                if (stack.isEmpty()){ // record the position before first left parenthesis
                    start=i+1;
                }else{
                    stack.pop();
                    // if stack is empty it means that the positon before the valid left parenthesis is "last"
                    if (stack.isEmpty()){
